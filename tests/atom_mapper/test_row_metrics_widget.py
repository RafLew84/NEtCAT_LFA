"""Tests for the row geometry-metrics widget."""
...
from AtomMapper.app.plots import PlotUnit, build_row_geometry_metrics
...
def test_row_metrics_widget_initializes_with_placeholder(qtbot):
    ...
    assert widget.title_label.text() == "Row geometry metrics [px]"
    assert "geometry metrics" in widget.placeholder_label.text().lower()


def test_row_metrics_widget_renders_geometry_summary(qtbot):
    widget = RowMetricsWidget()
    qtbot.addWidget(widget)

    metrics = build_row_geometry_metrics(_make_row_with_distances())
    ...
    assert widget.fitted_point_count_value.text() == "3"
    assert widget.spacing_count_value.text() == "2"
    assert widget.axis_angle_value.text() == "53.13 deg"
    assert widget.transverse_rms_value.text() == "0.000 px"
    assert widget.mean_spacing_value.text() == "7.500 px"
    assert widget.std_spacing_value.text() == "2.500 px"
...
def test_row_metrics_widget_switches_to_nm_summary(qtbot):
    ...
    assert widget.title_label.text() == "Row geometry metrics [nm]"
    assert widget.stack.currentWidget() is widget.metrics_panel
    assert widget.spacing_count_value.text() == "2"
    assert widget.axis_angle_value.text() == "53.13 deg"
    assert widget.transverse_rms_value.text() == "0.000 nm"
    assert widget.mean_spacing_value.text() == "0.750 nm"
    assert widget.std_spacing_value.text() == "0.250 nm"
...
def test_row_metrics_widget_shows_empty_state_for_too_few_points(qtbot):
    ...
    widget.set_metrics(build_row_geometry_metrics(row))
