...
class RealSpaceReconstructionDialog(QDialog):
    ...
    #. **Autocorrelation** – calculates the autocorrelation of the FFT power
       spectrum. This is useful for visualising real-space periodicities directly
       from the FFT.
    #. **Mask with ROIs** – lets the user draw rectangular regions of interest
       (ROIs) on the FFT. An inverse FFT is then performed on the masked data to
       reveal the real-space features corresponding to the selected frequencies.
    #. **Mask with Spots** – allows the user to select specific points (spots) on
       the FFT. A mask is generated by placing Gaussian profiles at these spots,
