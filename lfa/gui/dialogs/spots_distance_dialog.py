...
# except ImportError as e: # pragma: no cover
#     PEAK_FITTING_MODULE_AVAILABLE = False; SCIPY_AVAILABLE = False; convert_g_vector_px_to_nm_inv = None; calculate_real_space_vectors_from_g = None; HistoryNode = None; HistoryManager = None; apply_affine_transform = None;logging.error(f"SpotDistanceDialog: Error importing project modules: {e}")
...
#         self.selected_spots_fft_px_raw_refined: List[Tuple[float, float]] = [] 
...
#         self.spots_list_widget.clear() # Używaj self.spots_list_widget
#         if not self.selected_spots_fft_px_raw_refined:
#             self.spots_list_widget.addItem("No spots selected yet.")
#             return
        
#         for i in range(len(self.selected_spots_fft_px_raw_refined)):
#             raw_spot = self.selected_spots_fft_px_raw_refined[i]
            
#             corrected_spot_text = "N/A"
#             if i < len(self.corrected_spots_ideal_px) and self.corrected_spots_ideal_px[i] is not None:
#                 # Te są w idealnym systemie, do wyświetlenia w liście
#                 corr_kx, corr_ky = self.corrected_spots_ideal_px[i] # type: ignore
#                 corrected_spot_text = f"IdealFFT(kx={corr_kx:.1f}, ky={corr_ky:.1f} px)"
...
#                 f"Spot {i+1}: RawFFT({raw_spot[0]:.1f},{raw_spot[1]:.1f}) | {corrected_spot_text} | Dist: {distance_nm_str}"
#             )
...
#         if refined_spot_fft_px in self.selected_spots_fft_px_raw_refined:
#             self.status_label.setText(f"Spot ({refined_spot_fft_px[0]:.1f},{refined_spot_fft_px[1]:.1f}) already selected.")
#             return

#         self.selected_spots_fft_px_raw_refined.append(refined_spot_fft_px)
        
#         corrected_spot_ideal_system_px: Optional[Tuple[float, float]] = None
...
#                     corrected_spot_ideal_system_px = tuple(corrected_array[0])
#             except Exception as e:
#                 logger.error(f"Error correcting spot {refined_spot_fft_px} with substrate transform: {e}")
        
#         self.corrected_spots_ideal_px.append(corrected_spot_ideal_system_px) # Może być None

#         real_distance_nm: Optional[float] = None
#         if corrected_spot_ideal_system_px is not None: # Obliczaj tylko dla poprawnie skorygowanych
#             real_distance_nm = self._calculate_real_space_distance_for_spot(corrected_spot_ideal_system_px)
        
#         self.calculated_real_space_distances_nm.append(real_distance_nm)
        
#         self._update_spot_distance_list_widget()
#         self._redraw_all_markers_on_fft()
        
#         status_msg = f"Spot {len(self.selected_spots_fft_px_raw_refined)} added. "
#         if corrected_spot_ideal_system_px: status_msg += f"Corrected: ({corrected_spot_ideal_system_px[0]:.1f}, {corrected_spot_ideal_system_px[1]:.1f}). "
#         if real_distance_nm is not None: status_msg += f"Real Dist: {real_distance_nm:.3f} nm"
#         else: status_msg += "Real Dist: Error/Not corrected"
#         self.status_label.setText(status_msg)
#         self._update_add_spot_button_state() # Użyj _dist jeśli istnieje
...
#             # print(self.calculated_real_space_distances_nm)
#             self._update_spot_distance_list_widget()
...
#     @pyqtSlot()
#     def _remove_selected_spot_from_list(self): # Ujednolicona nazwa
#         current_row = self.spots_list_widget.currentRow()
#         if 0 <= current_row < len(self.selected_spots_fft_px_raw_refined):
#             del self.selected_spots_fft_px_raw_refined[current_row]
#             if current_row < len(self.corrected_spots_ideal_px): del self.corrected_spots_ideal_px[current_row]
#             if current_row < len(self.calculated_real_space_distances_nm): del self.calculated_real_space_distances_nm[current_row]
#             self._update_spot_distance_list_widget(); self._redraw_all_markers_on_fft(); logger.debug(f"Removed spot at idx {current_row}")
#             self._update_add_spot_button_state()

#     @pyqtSlot()
#     def _clear_all_listed_spots(self): # Ujednolicona nazwa
#         self.selected_spots_fft_px_raw_refined.clear(); self.corrected_spots_ideal_px.clear(); self.calculated_real_space_distances_nm.clear()
#         self._update_spot_distance_list_widget(); self._redraw_all_markers_on_fft(); logger.debug("Cleared all spots from distance list.")
#         self._update_add_spot_button_state()

#     def _update_add_spot_button_state(self): # Ujednolicona nazwa
#         self.add_spot_to_list_button.setEnabled(self.selection_roi.isVisible()) # Użyj self.selection_roi
#         self.status_label.setText(f"{len(self.selected_spots_fft_px_raw_refined)} spots selected. Click on FFT or use ROI, then 'Add Spot'.") # Użyj self.status_label
...
#     def _redraw_all_markers_on_fft(self): # Zmieniono nazwę dla spójności
#         # Usuń stare markery
#         if self.raw_refined_spot_markers: # Zmieniono nazwę z self.spot_markers_dist
#             try: self.fft_view_box.removeItem(self.raw_refined_spot_markers) # Użyj self.fft_view_box
#             except RuntimeError: pass
#             self.raw_refined_spot_markers = None
        
#         if self.corrected_spot_display_markers:
#             try: self.fft_view_box.removeItem(self.corrected_spot_display_markers)
#             except RuntimeError: pass
#             self.corrected_spot_display_markers = None

#         # 1. Rysuj surowe/uściślone piki (kliknięcia użytkownika)
#         if self.selected_spots_fft_px_raw_refined:
#             raw_spots_data = [{'pos': spot, 'symbol': 'o', 'size': 10, 
#                                'pen': pg.mkPen('orange', width=1.5), 
#                                'brush': pg.mkBrush(255,165,0,100)} 
#                               for spot in self.selected_spots_fft_px_raw_refined]
#             self.raw_refined_spot_markers = ScatterPlotItem(spots=raw_spots_data)
#             self.fft_view_box.addItem(self.raw_refined_spot_markers)

#         # 2. Rysuj skorygowane piki (przetransformowane z powrotem do przestrzeni obrazu FFT)
...
#             spots_to_display_on_fft = []
#             valid_corrected_for_transform = [s for s in self.corrected_spots_ideal_px if s is not None]
            
#             if valid_corrected_for_transform:
#                 try:
#                     F_inv = np.linalg.inv(self.sub_F_m2i)
#                     t_prime_for_display = (-self.sub_t_m2i @ F_inv.T).flatten() # type: ignore
                    
#                     transformed_back = apply_affine_transform(
#                         np.array(valid_corrected_for_transform, dtype=float), F_inv, t_prime_for_display
#                     )
#                     if transformed_back is not None:
#                         spots_to_display_on_fft = [tuple(pt) for pt in transformed_back]
#                 except Exception as e: logger.error(f"Error transforming corrected spots for display: {e}") # pragma: no cover

#             if spots_to_display_on_fft:
#                 corrected_markers_data = [{'pos': spot, 'symbol': 's', 'size': 10,
#                                            'pen': pg.mkPen('cyan', width=1.5), 
#                                            'brush': pg.mkBrush(0,255,255,100)} 
#                                           for spot in spots_to_display_on_fft]
#                 self.corrected_spot_display_markers = ScatterPlotItem(spots=corrected_markers_data)
#                 self.fft_view_box.addItem(self.corrected_spot_display_markers)
#         logger.debug("SpotDistanceDialog: Redrew spot markers.")
